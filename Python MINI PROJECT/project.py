...
for mastersheet_row_index in range(1, master_sheet.max_row + 1):
    for mastersheet_column_index in range(1, master_sheet.max_column + 1):
        master_sheet.cell(row=mastersheet_row_index,
                          column=mastersheet_column_index).value = None  # clearing all existing data present the mastersheet
...
for list_of_sheet_index in range(0, len(list_of_sheets)):
    if list_of_sheets[list_of_sheet_index] != wb['mastersheet'] and list_of_sheets[list_of_sheet_index] != wb['summarysheet']:
        ...
        for sheet_column_index in range(1, sheet_column + 1):
            master_sheet.cell(row=new_row_index, column=sheet_column_index).value = list_of_sheets[
                list_of_sheet_index].cell(row=1,
                                          column=sheet_column_index).value  # Copying all the headers from all the excel sheets into the mastersheet
        new_row_index += 1
                ...
                for sheet_column_index in range(1, sheet_column + 1):
                    master_sheet.cell(row=new_row_index, column=sheet_column_index).value = list_of_sheets[
                        list_of_sheet_index].cell(row=position,
                                                  column=sheet_column_index).value  # Copying values from each sheet to the mastersheet
                    count_column += 1
                count_row += 1
                new_row_index += 1
        new_row_index += 1
        ...
        continue
count_row = master_sheet.max_row
count_column = master_sheet.max_column - 1
...
new_summary_index += 1
summary_sheet.cell(row=new_summary_index, column=1).value = count_row  # Number of rows in the mastersheet
summary_sheet.cell(row=new_summary_index, column=2).value = count_column    # Number of columns in the mastersheet
